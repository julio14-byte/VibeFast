...
export function generarCfdiXml({
  ...
  const conceptosXml = conceptos
    .map(
      (c) =>
        `<cfdi:Concepto ClaveProdServ="${c.clave_sat}" Cantidad="${c.cantidad}" ClaveUnidad="${c.unidad_sat}" Descripcion="${escapeXml(c.nombre)}" ValorUnitario="${c.precio_unitario.toFixed(2)}" Importe="${c.subtotal.toFixed(2)}" ObjetoImp="02"/>`
    )
    .join("\n    ")
